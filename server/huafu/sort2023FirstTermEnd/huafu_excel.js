...
const excelSort = async function(src,dst){
    ...
    console.log('workbook.worksheets.length', workbook.worksheets.length);
    workbook.eachSheet(function(worksheet, sheetId) {
        worksheet.eachRow((row, rowNumber) => {
            if (sheetId === 3) {
                ...
                //         // sheet1: colNumber 1班级 2姓名 15总分 16校排名
                //         // sheet2/3: colNumber 1班级 2姓名 17总分 18校排名
                //         let value = cell.value;
                //         console.log('colNumber:', colNumber, 'value', value)
                //     });
                // }
                // 从第2行开始
                if (rowNumber >= 2) {
    ...
    fs.writeFile('data_huafu_sheet3.json', JSON.stringify(result), (err) => {
        if (err) throw err;
